...
def main() -> int:
    ...
    # ----- preserve manualEdits from previous run --------------------------
    # Each story may carry a `manualEdits: [...]` array listing fields that
    # were hand-tuned post-crawl (e.g. cover replaced for IP safety, lead
    # rewritten to scrub a real name from an anonymous story, etc.).
    # We re-read the previous stories-data.js (if any), and for every chosen
    # story carry the locked field values forward so this script never
    # silently undoes those edits.
    if OUT.exists():
        try:
            prev_raw = OUT.read_text(encoding="utf-8")
            mprev = re.search(r"window\.STORIES\s*=\s*(\[.*\]);", prev_raw, re.S)
            prev_list = json.loads(mprev.group(1)) if mprev else []
            prev_by_slug = {p.get("slug"): p for p in prev_list if p.get("slug")}
        except Exception as e:
            print(f"  ! could not parse existing stories-data.js for merge: {e}")
            prev_by_slug = {}

        restored = 0
        for r in chosen:
            prev = prev_by_slug.get(r.get("slug"))
            if not prev:
                continue
            locks = prev.get("manualEdits") or []
            if not locks:
                continue
            for field in locks:
                if field in prev:
                    r[field] = prev[field]
                    restored += 1
            r["manualEdits"] = sorted(set(locks))
            # Carry forward the rescue copy of the real name if present
